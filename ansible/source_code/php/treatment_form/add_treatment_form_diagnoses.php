...
<body style="padding-top: 70px;">
  ...
  <div id="container">
	  ...
	  <div style="float: left;  width: 300px;">
        <form method="post" action="add_treatment_form.php">
		...
		if ($_POST['diagnosis']) {
			...
			if ($_POST['add'] == 'yes') {
				...
				if ($count) {
					...
					$stmt = $conn->prepare("SELECT COUNT(*) FROM diagnoses_temp WHERE diagnosis='$diagnosis' AND created_by='$username' AND form_type='treatment'");
					$stmt->execute();
					$result = $stmt->setFetchMode(PDO::FETCH_ASSOC);
					...
					if ($count == 0) {
						...
						// attempt to add it
						try {
							$query = "INSERT INTO diagnoses_temp (diagnosis, created_by, form_type) VALUES ('$diagnosis', '$username', 'treatment');"; 
							$conn->exec($query);
						}
			...
			// if clinician is not adding diagnosis, then they are removing it
				...
				try {
					$query = "DELETE FROM diagnoses_temp WHERE diagnosis='$diagnosis' and created_by='$username' AND form_type='treatment';"; 
					$conn->exec($query);
				}
   ...
   <!-- start of bottom card -->
    <div class="accountCard" style="float: left; margin-right: 500px;width: 400px; height: 250px; position: relative;">
		...
		<div style=' margin-left:25px;'>
				...
				<form action="add_treatment_form_diagnoses.php" method="post" >
		...
		<div>
			...
			<div style='position: absolute; top: 105px; padding-left: 50px; height: 150px; overflow:auto;'>
				...
				<?php
						$stmt = $conn->prepare("SELECT diagnosis FROM diagnoses_temp  WHERE created_by='$username' AND form_type='treatment';");
						$stmt->execute();
						$result = $stmt->setFetchMode(PDO::FETCH_ASSOC);
		...
		<div style="margin-top: 160px; margin-left: 100px;">	
			<form action="insert_treatment_form.php">
