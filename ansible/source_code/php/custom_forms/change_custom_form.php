...
<body style="padding-top: 70px;">
  ...
  <div id="container">
	  ...
	  <div style="float: left;  width: 400px;">
        <form method="post" action="/php/custom_forms/select_custom_form_individual_form.php">
            <input style="width: 400px;" type="submit" value="Back to Form Selection">
        </form>
      </div>
				...
				for($i = 0; $i < $table_columns_max; ++$i) {
					...
					if ($start_found) {
								...
								case 'image-upload':
									...
									switch ($attribute_name_array[3]) {
										...
										case 'medium':
											...
											break;
										case 'large':
											array_push($form_array, "<div style='height: 800px; '><img style='height: 800px; width: 800px; ' src='$next_value' alt='unable to load image'></div>");
											break;
										case 'large-short':
											array_push($form_array, "<div style='height: 400px;'><img style='height: 400px; width: 800px; ' src='$next_value' alt='unable to load image'></div>");
											break;
									}
									break;
								case 'textbox':
									...
									do {
										...
										$current_column_query = query_format($table_columns[$i]);
										$column_label = str_replace("_{$attribute_value}", '', $current_column);
										$column_label =  html_format($column_label);
										$html_name = html_name_format($table_columns[$i]);
										
										$stmt = $conn->prepare("SELECT `$current_column_query` FROM $table_name WHERE $table_id = $choosen_custom_form_id;");
										$stmt->execute();
										$result = $stmt->setFetchMode(PDO::FETCH_ASSOC);
										...
										$textbox_value = html_value_format($textbox_value);

										$textbox_array_temp = $textbox_array_temp . "<div style='height: 50px; float: left;margin-left: 15px;'>$column_label: <input style='height: 35px; width: 100px;' type='text' value='$textbox_value' name='$html_name' maxlength='25' $auto_focus></div>";
										++$i;
						...
						if ($current_column_type == "enum('yes','no')") {
							...
							array_push($form_array, "<div style='height: 40px;'><input type='hidden' name='$html_name' value='no'><input type='checkbox' name='$html_name' value='yes' $checked>$column_label </div>");
						...
						// radio_button_group
							...
							foreach ($radio_button_array as &$current_button) {
								...
								$temp_html = $temp_html . "<input type='radio' name='$html_name' value='$current_button' $is_checked>$current_button ";
						...
						switch ($current_column_type) {
						    case 'varchar(50)':
							array_push($form_array,"<div style='height: 50px;'>$column_label: <input style='height: 35px; width: 500px;' type='text' name='$html_name' value='$value' maxlength='50' $auto_focus></div>");
							break;
						    case 'int(11)':
							array_push($form_array,"<div style='height: 50px;'>$column_label: <input style='height: 35px; width: 150px;' type='number' name='$html_name' value='$value' $auto_focus></div>");
							break;
						    case 'tinytext':
							array_push($form_array,"<div style='height: 200px;'>$column_label: <br><textarea style='height: 150px; width: 500px;' name='$html_name' maxlength='255' $auto_focus>$value</textarea></div>");
							break;
						    case 'text':
							array_push($form_array,"<div style='height: 300px;'>$column_label: <br><textarea style='height: 250px; width: 800px;' name='$html_name' maxlength='5000' $auto_focus>$value</textarea></div>");
							
							break;
						    case 'varchar(1000)':
							array_push($form_array,"<div style='margin-left: 80px; margin-top: 10px;height: 80px;'>Add/Change Image <br><input type='file' name='$html_name' id='$current_column'><br><label style='font-size: 12px;'>(jpg, jpeg, png, or gif,  20MB max)</label></div>");
							break;
						    case 'date':
							array_push($form_array,"<div style='height: 50px;margin-top: 15px;'>$column_label: <input style='height: 25px; width: 150px;' type='date' name='$html_name' value='$value' $auto_focus></div>");
							break;
						}
