...
try {
	...
	if ($account_type == 'program') {
		...
		/* create new connection */
	    	$conn = new PDO($dbconnection_radius, $dbusername_radius, $dbpassword_radius);
	    	$conn->setAttribute(PDO::ATTR_EMULATE_PREPARES, false);
		$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		...
		if (password_check($new_password, $confirm_password)) {
			...
			$stmt = $conn->prepare("UPDATE `radcheck` SET `value`=:new_password WHERE `username`=:username;");
			$stmt->execute(array('new_password' => $new_password, 'username' => $_SESSION['username']));
