...
// if the account exists
	...
	if ($vigina_path_check != 'no_image' and $vigina_path != 'no_image'){
		rename($vigina_path_check, '../../../uploaded_images/no_longer_used/women_health_images_history/vigina_images_history/' . substr($vigina_path_check, strposX($vigina_path_check, '/', 7) + 1));
	}	
	...
	if ($breast_path_check != 'no_image' and $breast_path != 'no_image'){
		rename($breast_path_check, '../../../uploaded_images/no_longer_used/women_health_images_history/breast_images_history/' . substr($breast_path_check, strposX($breast_path_check, '/', 7) + 1));
	}	
	// make sure the image is not replaced, if an image exists in the database
	...
	if ($circle_path_check != 'no_image' and $circle_path != 'no_image'){
		rename($circle_path_check, '../../../uploaded_images/no_longer_used/women_health_images_history/circle_images_history/' . substr($circle_path_check, strposX($circle_path_check, '/', 7) + 1));
	}	
	// make sure the image is not replaced, if an image exists in the database
	...
	try {
		...
		create_database_error($query, 'update_women_health.php (client)', $e->getMessage());	
	}
