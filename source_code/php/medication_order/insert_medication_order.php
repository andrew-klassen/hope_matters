...
$_SESSION['to'] = $to;
	...
	try {
		//if ($t != "" and $bp != "" and $hr != "" and $sao2 != "") {
			...
			$query = "INSERT INTO medication_order (client_id, first_name, last_name, sex, location, date_of_birth, medication, dosage, frequency, administration_method, notes, created_by) 
			VALUES ('$choosen_client_id', '$first_name', '$last_name', '$sex', '$location', '$date_of_birth', '$medication', '$dosage', '$frequency', '$administration_method', '$notes', '$username');"; 
			$conn->exec($query);
			
			// redirect user back to discharge form selection
			header( 'Location: add_medication_order.php');
			exit();
		...
		*/
		...
		create_database_error($query, 'insert_medication_order.php', $e->getMessage());
	}
