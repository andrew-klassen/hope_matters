...
class view_clients extends RecursiveIteratorIterator {
    ...
    function current() {
		...
		return "<td style='border-style: solid; border-color: #black; background-color: white; color: black;font-weight: 500;font-size: 12px;font-size: 30px;  '>" . "<a href='grab_choosen_client_id_women_health.php? choosen_client_id=$temp' style='color: black;'>"  . parent::current() . "</a>" . "</td>";
...
try {
	...
	echo "<p style='color: black;text-align: center;'>Click on a client to create a Women's Health Report for them.</p>";
	...
	<form action='select_client_women_health.php' method='post' id='searchForm'>
	...
	if (ctype_digit($search)) {
		...
		$stmt = $conn->prepare("SELECT client_id, first_name, last_name, sex, DATE_FORMAT(NOW(), '%Y') - DATE_FORMAT(date_of_birth, '%Y') - (DATE_FORMAT(NOW(), '00-%m-%d') < DATE_FORMAT(date_of_birth, '00-%m-%d')), date_format(timestamp, '%b %d %Y %h:%i %p') FROM general_info WHERE client_id='$search' AND sex='female' $query_date_of_birth;");
		$stmt->execute();
	...
	elseif ($search == '*') {
		...
		$stmt = $conn->prepare("SELECT client_id, first_name, last_name, sex, DATE_FORMAT(NOW(), '%Y') - DATE_FORMAT(date_of_birth, '%Y') - (DATE_FORMAT(NOW(), '00-%m-%d') < DATE_FORMAT(date_of_birth, '00-%m-%d')), date_format(timestamp, '%b %d %Y %h:%i %p') FROM general_info WHERE sex='female' $query_date_of_birth ORDER BY client_id DESC LIMIT $wild_card_limit;");
		$stmt->execute();
		$result = $stmt->setFetchMode(PDO::FETCH_ASSOC);
	...
	elseif ($search) {
		...
		$stmt = $conn->prepare("SELECT client_id, first_name, last_name, sex, DATE_FORMAT(NOW(), '%Y') - DATE_FORMAT(date_of_birth, '%Y') - (DATE_FORMAT(NOW(), '00-%m-%d') < DATE_FORMAT(date_of_birth, '00-%m-%d')), date_format(timestamp, '%b %d %Y %h:%i %p') FROM general_info WHERE concat(first_name, ' ', last_name) LIKE '%$search%' AND sex='female' $query_date_of_birth ORDER BY CASE WHEN concat(first_name, ' ', last_name) LIKE '$search' THEN 1 ELSE 2 END, client_id desc LIMIT $search_limit;");
		$stmt->execute();
		$result = $stmt->setFetchMode(PDO::FETCH_ASSOC);
