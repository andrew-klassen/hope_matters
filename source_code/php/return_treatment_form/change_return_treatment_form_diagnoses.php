...
<body style="padding-top: 70px;">
  ...
  <div id="container">
	  ...
	  <div style="float: left;  width: 300px;">
        <form method="post" action="change_return_treatment_form.php">
		...
		if ($_POST['diagnosis']) {
			...
			if ($_POST['add'] == 'yes') {
				...
				if ($count) {
					...
					$stmt = $conn->prepare("SELECT COUNT(*) FROM diagnoses WHERE diagnosis='$diagnosis' and form_id='$choosen_return_treatment_form_id' AND form_type='return_treatment'");
					$stmt->execute();
					$result = $stmt->setFetchMode(PDO::FETCH_ASSOC);
					...
					if ($count == 0) {
						...
						// attempt to add it
						try {
							$query = "INSERT INTO diagnoses (client_id, form_id, first_name, last_name, sex, location, date_of_birth, diagnosis, clinician, created_by, form_type) VALUES ('$client_id', '$choosen_return_treatment_form_id', '$first_name', '$last_name', '$sex', '$location', '$date_of_birth', '$diagnosis', '$clinician', '$username', 'return_treatment');"; 
							$conn->exec($query);
						}
			...
			// if clinician is not adding diagnosis, then they are removing it
				...
				// try to remove diagnosis
					try {
						...
						$stmt = $conn->prepare("SELECT diagnosis_id FROM diagnoses WHERE diagnosis='$diagnosis' AND form_id='$choosen_return_treatment_form_id' AND form_type='return_treatment'");
						$stmt->execute();
						$result = $stmt->setFetchMode(PDO::FETCH_ASSOC);
						...
						$stmt = $conn->prepare("SELECT timestamp FROM diagnoses WHERE diagnosis='$diagnosis' AND form_id='$choosen_return_treatment_form_id' AND form_type='return_treatment'");
						$stmt->execute();
						$result = $stmt->setFetchMode(PDO::FETCH_ASSOC);
						...
						$query = "INSERT INTO diagnoses_history (diagnosis_id, client_id, form_id, first_name, last_name, sex, location, date_of_birth, diagnosis, clinician, timestamp, created_by, form_type) VALUES ('$diagnosis_id', '$client_id', '$choosen_return_treatment_form_id', '$first_name', '$last_name', '$sex', '$location', '$date_of_birth', '$diagnosis', '$clinician', '$timestamp', '$username', 'return_treatment');"; 
						$conn->exec($query);
					
						$query = "DELETE FROM diagnoses WHERE diagnosis='$diagnosis' AND form_type='return_treatment';"; 
						$conn->exec($query);
   ...
   <!-- start of bottom card -->
    <div class="accountCard" style="float: left; margin-right: 500px;width: 400px; height: 250px; position: relative;">
		...
		<div style=' margin-left:25px;'>
				...
				<form action="change_return_treatment_form_diagnoses.php" method="post" >
		...
		<div>
			...
			<div style='position: absolute; top: 105px; padding-left: 50px; height: 150px; overflow:auto;'>
				...
				<?php
						$stmt = $conn->prepare("SELECT diagnosis FROM diagnoses WHERE form_id='$choosen_return_treatment_form_id' AND form_type='return_treatment';");
						$stmt->execute();
						$result = $stmt->setFetchMode(PDO::FETCH_ASSOC);
		...
		<!-- submit button -->
		<div style="margin-top: 160px; margin-left: 80px;">	
			<form action="select_return_treatment_form.php">
